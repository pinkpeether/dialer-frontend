...
export default function KeyboardShortcutsPanel({ shortcuts, enabled, onToggleShortcut }: Props) {
  return (
    <section
      style={{
    ...
    >
      <div style={{ marginBottom: 14 }}>
        ...
        <p style={{ margin: '6px 0 0', color: '#cbd5e1' }}>
          Fast call-floor controls for mute, hold, hangup, dialer, and disposition saving.
        </p>
      </div>

      {!enabled && (
        <div style={{ marginBottom: 12, color: '#fbbf24', fontWeight: 700 }}>
          Shortcuts are currently disabled in preferences.
        </div>
      )}
          ...
          <tbody>
            {shortcuts.map((shortcut) => (
              <tr key={shortcut.action} style={{ borderTop: '1px solid rgba(148, 163, 184, 0.16)' }}>
                ...
                <td style={tdStyle}>
                  <button type="button" onClick={() => onToggleShortcut(shortcut.action)} style={buttonStyle}>

...
const NAV: NavItem[] = [
  ...
  { to: '/advanced-dialing', icon: Zap,            label: 'Advanced Dialing', roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'SUPERVISOR'], color: COLORS.orange },
  ...
  { to: '/campaign-management-pro', icon: Layers3, label: 'Campaign Management Pro', roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'SUPERVISOR'], color: COLORS.teal },
  ...
  { to: '/ui-ux-pro',       icon: Palette,         label: 'UI/UX Pro',      roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'SUPERVISOR'], color: COLORS.teal },
  { to: '/security-admin-pro', icon: LockKeyhole,  label: 'Security Admin Pro', roles: ['SUPER_ADMIN', 'ADMIN'], color: COLORS.red },
  { to: '/deployment-platform-pro', icon: ServerCog, label: 'Deployment Platform', roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'SUPERVISOR'], color: COLORS.teal },
  ...
  { to: '/recording-storage-pro', icon: HardDriveDownload, label: 'Recording Storage', roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'SUPERVISOR'], color: COLORS.cyan },
...
const CONSOLE_GROUPS: NavGroup[] = [
  ...
  { key: 'administration', label: 'ADMINISTRATION', icon: Crown, color: COLORS.pink, roles: ['SUPER_ADMIN', 'ADMIN'], items: ['/platform/administration', '/commercial-control', '/admin/spoofing'] },
  { key: 'billing', label: 'BILLING', icon: Building2, color: COLORS.gold, roles: ['SUPER_ADMIN', 'ADMIN', 'CUSTOMER_ADMIN', 'MANAGER', 'SUPERVISOR'], items: ['/billing'] },
...
const AGENT_GROUPS: NavGroup[] = [
  { key: 'dashboard', label: 'Workspace', icon: BriefcaseBusiness, color: COLORS.purple, items: ['/agent/workspace'] },
...
export default function Sidebar() {
  ...
  const itemMap = useMemo(() => {
    const source = [...NAV, ...AGENT_NAV]
    return Object.fromEntries(source.map(item => [item.to, item])) as Record<string, NavItem>
  }, [])
    ...
    return sourceGroups
      ...
      .map(group => ({
        ...
        navItems: group.items
          .map(to => itemMap[to])
          .filter(Boolean)
          .filter(item => isVisibleForRole(item.roles)),
      }))
  ...
  useEffect(() => {
    setExpandedGroups(prev => {
      ...
      groups.forEach(group => {
        const hasActiveChild = group.navItems.some(item => isPathActive(item.to))
        if (hasActiveChild && !next[group.key]) {
          next[group.key] = true
          changed = true
        }
      })
  ...
  return (
    <>
      ...
      <aside className={`ptdt-sidebar ${mobileOpen ? 'is-open' : ''}`} style={{
        ...
        <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: '10px 6px 18px', marginBottom: 10 }}>
          <button
            type="button"
          ...
          <motion.img src="ptdt-main-logo.png" alt="PTDT" whileHover={{ scale: 1.06 }} transition={{ type: 'spring', stiffness: 280 }} style={{ width: 68, height: 68, objectFit: 'contain', borderRadius: 16, background: 'transparent', mixBlendMode: 'multiply', marginBottom: 10 }} />
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 900, color: 'var(--text)', lineHeight: 1.05, letterSpacing: '-0.04em' }}>
            PTDT-<span className="gradient-brand-text">Dialer</span>
          </div>
          <div className="mono" style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: 1.8, marginTop: 5, fontWeight: 700 }}>
        ...
        <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto', overflowX: 'hidden', paddingRight: 2 }}>
          ...
          {(() => {
            ...
            return groups.map(group => {
              ...
              // Dim non-open groups when any group is open (but never dim the open one)
              const isDimmed = anyOpen && !isOpen

              return (
                <div key={group.key} style={{ display: 'grid', gap: 5, opacity: isDimmed ? 0.42 : 1, transition: 'opacity .25s ease' }}>
                  <button
                    type="button"
                    ...
                    style={{
                      ...
                      padding: '9px 12px',
                      borderRadius: 14,
                      border: `1px solid ${hexToRgba(groupColor, isOpen ? 0.52 : isGroupActive ? 0.35 : 0.18)}`,
                      background: isOpen
                        ? `linear-gradient(135deg, ${hexToRgba(groupColor, 0.32)}, ${hexToRgba(groupColor, 0.16)})`
                        : isGroupActive
                          ? `linear-gradient(135deg, ${hexToRgba(groupColor, 0.18)}, ${hexToRgba(groupColor, 0.08)})`
                          : `linear-gradient(135deg, ${hexToRgba(groupColor, 0.09)}, ${hexToRgba(groupColor, 0.04)})`,
                      color: groupColor,
                      cursor: 'pointer',
                      textAlign: 'left',
                      boxShadow: isOpen
                        ? `0 6px 18px ${hexToRgba(groupColor, 0.28)}, inset 0 1px 0 ${hexToRgba(groupColor, 0.3)}`
                        : `0 1px 4px ${hexToRgba(groupColor, 0.08)}`,
                      transition: 'all .25s ease',
                      transform: isOpen ? 'scale(1.01)' : 'scale(1)',
                    }}
                  >
                    <span className="sidebar-icon-shell" style={{ color: groupColor, background: hexToRgba(groupColor, isOpen ? 0.2 : 0.1), boxShadow: isOpen ? `0 2px 8px ${hexToRgba(groupColor, 0.35)}` : 'none' }}>
                      <Icon size={16.5} strokeWidth={isOpen ? 2.5 : 2.1} />
                    </span>
                  ...
                  {isOpen && (
                    <div style={{ display: 'grid', gap: 4, paddingLeft: 10, marginLeft: 13, borderLeft: `2px solid ${hexToRgba(groupColor, 0.32)}` }}>
                      {group.navItems.map(item => {
                        const ItemIcon = item.icon
                        // FIX: Always use groupColor for icon — this is what creates synergy
                        return (
                            ...
                            {({ isActive }) => (
                              <motion.div
                                ...
                                style={{
                                  ...
                                  padding: '7px 10px', borderRadius: 12, fontSize: 12.5, fontWeight: 700,
                                  background: isActive ? `linear-gradient(135deg, ${hexToRgba(groupColor, 0.22)}, ${hexToRgba(groupColor, 0.10)})` : 'transparent',
                                  color: isActive ? groupColor : hexToRgba(groupColor, 0.65),
                                  border: isActive ? `1px solid ${hexToRgba(groupColor, 0.35)}` : '1px solid transparent',
                                  boxShadow: isActive ? `0 6px 16px ${hexToRgba(groupColor, 0.15)}, inset 0 1px 0 ${hexToRgba(groupColor, 0.2)}` : 'none',
                                  transition: 'background .22s, color .22s, box-shadow .22s, border-color .22s',
                                }}
                              >
                                <span className="sidebar-icon-shell" style={{ color: isActive ? groupColor : hexToRgba(groupColor, 0.72), background: isActive ? hexToRgba(groupColor, 0.12) : undefined }}>
                                  <ItemIcon size={15.5} strokeWidth={isActive ? 2.35 : 2.05} />
                                </span>
                                <span style={{ overflow: 'visible', textOverflow: 'clip', whiteSpace: 'normal', lineHeight: 1.25, flex: 1 }}>{item.label}</span>
                              </motion.div>
                            )}
                          </NavLink>
                        )
                    ...
                    {({ isActive }) => (
                      <motion.div
                        ...
                        style={{
                          ...
                          background: isActive ? `linear-gradient(135deg, ${hexToRgba(iconColor, 0.26)}, ${hexToRgba(iconColor, 0.12)})` : 'transparent',
                          color: isActive ? iconColor : hexToRgba(iconColor, 0.8),
                          border: isActive ? `1px solid ${hexToRgba(iconColor, 0.3)}` : '1px solid transparent',
                          boxShadow: isActive ? `0 10px 22px ${hexToRgba(iconColor, 0.16)}, inset 0 1px 0 ${hexToRgba(iconColor, 0.2)}` : 'none',
                          transition: 'background .22s, color .22s, box-shadow .22s, border-color .22s',
                        }}
                      >
                        <span className="sidebar-icon-shell" style={{ color: isActive ? iconColor : hexToRgba(iconColor, 0.84), background: isActive ? hexToRgba(iconColor, 0.12) : undefined }}>
                          <Icon size={16.5} strokeWidth={isActive ? 2.35 : 2.2}/>
                        </span>
                        <span style={{ overflow: 'visible', textOverflow: 'clip', whiteSpace: 'normal', lineHeight: 1.25, flex: 1 }}>{item.label}</span>
                      </motion.div>
                    )}

...
function StateBanner({ type, children }: { type: 'error' | 'success'; children: ReactNode }) {
  ...
  return (
    <div
      style={{
        ...
        marginBottom: 16,
        borderRadius: 'var(--radius-md)',
        border: isError ? '1px solid rgba(239,68,68,0.28)' : '1px solid rgba(0,167,71,0.28)',
        background: isError ? 'rgba(239,68,68,0.08)' : 'rgba(0,167,71,0.08)',
        color: isError ? 'var(--danger)' : 'var(--green-2)',
        fontSize: 13,
        fontWeight: 800,
      }}
    >
          ...
          <>
            ...
            >
              <input
                type="number"
                min={1}
                value={Number(form.defaultDialingRatio || 1)}
            ...
            >
              <input
                type="number"
                min={1}
                value={Number(form.defaultRetryDelayMinutes || 60)}
            ...
            >
              <input
                type="number"
                min={0}
                value={Number(form.defaultMaxRetries || 3)}
            ...
            >
              <input
                type="number"
                min={5}
                value={Number(form.callTimeoutSeconds || 45)}
            ...
            >
              <input
                type="number"
                min={1}
                value={Number(form.callbackReminderMinutes || 15)}
            ...
            >
              <input
                type="number"
                min={0}
                value={Number(form.lowContactThreshold || 25)}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 20 }}>
              ...
              <button
                ...
                disabled={saving}
                onClick={() => void save()}
                style={{ height: 43, borderRadius: 999, padding: '0 24px', display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 900, opacity: saving ? 0.7 : 1 }}
              >

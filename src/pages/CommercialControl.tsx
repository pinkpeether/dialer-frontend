...
export default function CommercialControl() {
  ...
  const selectedPaymentAddonName = paymentForm.requestedAddonCodes[0]
    ? catalog?.addons.find(addon => addon.code === paymentForm.requestedAddonCodes[0])?.name || cleanDisplayText(paymentForm.requestedAddonCodes[0])
    : 'No add-on'
      ...
      {summary && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 14, marginBottom: 18 }}>
            ...
            <div className="glass" style={cardStyle}><div className="eyebrow pink"><WalletCards size={12} /> Calling Wallet</div><div style={{ fontSize: 26, fontWeight: 950, color: 'var(--text)', marginTop: 8 }}>{money(summary.wallet?.availableBalance, currentCurrency)}</div><div className="mono" style={{ color: stateColor(summary.balanceState), marginTop: 6, fontWeight: 900 }}>{cleanDisplayText(summary.balanceState)}</div></div>

...
export default function TeamUsersV3() {
  ...
  const renderUserRow = (user: TeamUser) => {
    ...
    return <tr key={Number(user.id)} style={{ borderBottom: '1px solid var(--border)', minHeight: 102 }}>
      <td style={{ ...teamUserCellStyle(0), fontWeight: 900, fontSize: 18.5, lineHeight: 1.2 }}>{editingId === Number(user.id) ? <input value={editingName} onChange={event => setEditingName(event.target.value)} style={{ ...inputStyle, maxWidth: 260, minHeight: 38 }} autoFocus /> : String(user.name || '—')}<br /><span className="mono" style={{ color: 'var(--text-3)', fontSize: 12.2 }}>{String(user.agentCode || '')}</span></td>
      <td style={{ ...teamUserCellStyle(1), fontSize: 18.5, fontWeight: 780 }}>{String(user.email || '—')}</td>
      <td style={{ ...teamUserCellStyle(2), fontSize: 17.4 }}><span style={roleStyleFor(user.role, user.status, active)}>{roleLabel(user.role)}</span></td>
      <td style={teamUserCellStyle(3)}><span className="badge" style={{ color: statusStyle.color, background: statusStyle.bg, border: `1px solid ${statusStyle.color}`, fontSize: 12.2 }}>{presenceLabel(user.status, active)}</span></td>
      <td style={teamUserCellStyle(4)}><span className="badge" style={{ color: dialerStyle.color, background: dialerStyle.bg, border: `1px solid ${dialerStyle.color}`, fontSize: 12.2 }}>{dialerStatusLabel(user.status, active)}</span></td>
      <td style={teamUserCellStyle(5)}>{isSelf ? <span className="badge" style={{ color: green, background: 'rgba(0,167,71,.10)', border: '1px solid rgba(0,167,71,.28)', fontSize: 12 }}><ShieldCheck size={13} /> Signed in</span> : canToggleActive ? <button type="button" role="switch" aria-checked={active} disabled={pendingId === Number(user.id)} onClick={() => void setUserActive(user, !active)} style={switchStyle(active, pendingId === Number(user.id))}><span style={switchKnob} /></button> : <span className="badge" style={{ color: 'var(--text-3)', background: 'var(--bg-2)', border: '1px solid var(--border)', fontSize: 12 }}>Protected</span>}</td>

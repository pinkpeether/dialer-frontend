...
const teamUserColumnWidths = ['19%', '24%', '14%', '12%', '14%', '8%', '9%']
const teamUserCellPadding = '22px 18px'
const teamUserHeaderStyle = (index: number): React.CSSProperties => ({
  width: teamUserColumnWidths[index],
...
const teamUserCellStyle = (index: number): React.CSSProperties => ({
  padding: teamUserCellPadding,
  textAlign: index <= 1 ? 'left' : 'center',
  verticalAlign: 'middle',
})
...
const liveStatus = (status: unknown) => String(status || 'OFFLINE').toUpperCase()

const isPresenceOnline = (status: unknown, active: boolean) => {
  if (!active) return false
  return ['ONLINE', 'READY', 'BUSY', 'WRAP_UP'].includes(liveStatus(status))
}

const presenceLabel = (status: unknown, active: boolean) => {
  if (!active) return 'Disabled'
  return isPresenceOnline(status, active) ? 'Online' : 'Offline'
}

const presenceStyleFor = (status: unknown, active: boolean) => {
  if (isPresenceOnline(status, active)) return { color: green, bg: 'rgba(0,167,71,.10)' }
  return { color: 'var(--text-3)', bg: 'var(--bg-2)' }
}

const dialerStatusLabel = (status: unknown, active: boolean) => {
  if (!active) return 'Disabled'
  const normalized = liveStatus(status)
  if (normalized === 'READY') return 'Ready'
  if (normalized === 'BUSY') return 'Busy'
  if (normalized === 'WRAP_UP') return 'Wrap-up'
  if (normalized === 'ONLINE') return 'Not Ready'
  return 'Offline'
}

const roleStyleFor = (role: unknown, status: unknown, active: boolean): React.CSSProperties => {
  if (role === 'SUPERVISOR') {
    const online = isPresenceOnline(status, active)
    return {
      color: online ? green : 'var(--text-3)',
      background: online ? 'rgba(0,167,71,.10)' : 'var(--bg-2)',
      border: `1px solid ${online ? 'rgba(0,167,71,.30)' : 'var(--border)'}`,
      borderRadius: 999,
      padding: '5px 9px',
      display: 'inline-flex',
      width: 'fit-content',
  ...
  return { color: 'var(--text)', fontWeight: 850 }
}
...
export default function TeamUsersV3() {
  ...
  const renderUserRow = (user: TeamUser) => {
    ...
    return <tr key={Number(user.id)} style={{ borderBottom: '1px solid var(--border)', minHeight: 112 }}>
      <td style={{ ...teamUserCellStyle(0), fontWeight: 950, fontSize: 32, lineHeight: 1.13 }}>{editingId === Number(user.id) ? <input value={editingName} onChange={event => setEditingName(event.target.value)} style={{ ...inputStyle, maxWidth: 260, minHeight: 38 }} autoFocus /> : String(user.name || '—')}<br /><span className="mono" style={{ color: 'var(--text-3)', fontSize: 14.5, fontWeight: 850, letterSpacing: 0.8 }}>{String(user.agentCode || '')}</span></td>
      <td style={{ ...teamUserCellStyle(1), fontSize: 32, fontWeight: 850 }}>{String(user.email || '—')}</td>
      <td style={{ ...teamUserCellStyle(2), fontSize: 29 }}><span style={roleStyleFor(user.role, user.status, active)}>{roleLabel(user.role)}</span></td>
      <td style={teamUserCellStyle(3)}><span className="badge" style={{ color: statusStyle.color, background: statusStyle.bg, border: `1px solid ${statusStyle.color}`, fontSize: 20 }}>{presenceLabel(user.status, active)}</span></td>
      <td style={teamUserCellStyle(4)}><span className="badge" style={{ color: dialerStyle.color, background: dialerStyle.bg, border: `1px solid ${dialerStyle.color}`, fontSize: 20 }}>{dialerStatusLabel(user.status, active)}</span></td>
      <td style={teamUserCellStyle(5)}>{isSelf ? <span className="badge" style={{ color: green, background: 'rgba(0,167,71,.10)', border: '1px solid rgba(0,167,71,.28)', fontSize: 19.5 }}><ShieldCheck size={13} /> Signed in</span> : canToggleActive ? <button type="button" role="switch" aria-checked={active} disabled={pendingId === Number(user.id)} onClick={() => void setUserActive(user, !active)} style={switchStyle(active, pendingId === Number(user.id))}><span style={switchKnob} /></button> : <span className="badge" style={{ color: 'var(--text-3)', background: 'var(--bg-2)', border: '1px solid var(--border)', fontSize: 19.5 }}>Protected</span>}</td>
  ...
  const tableHeader = <thead><tr>{['USER', 'EMAIL', 'ROLE', 'STATUS', 'DIALER STATUS', 'ACTIVE', 'ACTIONS'].map((label, index) => <th key={label} style={teamUserHeaderStyle(index)}>{label}</th>)}</tr></thead>

...
@media (max-width: 900px) {
  ...
  .ptdt-mobile-page .recharts-wrapper, .ptdt-mobile-page .recharts-surface, .ptdt-mobile-page .recharts-responsive-container { max-width: 100% !important; }
  .ptdt-mobile-page .recharts-legend-wrapper { max-width: 100% !important; }
...
type CustomerCallGroup = CustomerAccount & { key: string; calls: CallRow[] }
...
function accountForCall(row: Record<string, unknown>): CustomerAccount {
...
function normalizeCall(item: unknown, index: number): CallRow {
  ...
  const dynamicText = [remoteName, remoteNumber, stringValue(row.notes), nestedName(contact), stringValue(row.name)].join(' ').toLowerCase()
...
function normalizeResponse(payload: unknown, page: number, limit: number): PagedResponse {
  ...
  return {
    ...
    total: numberValue(record.total ?? record.count ?? (isRecord(record.pagination) ? record.pagination.total : undefined), items.length),
  }
}
...
function callHistoryErrorMessage(err: unknown) {
  if (isRecord(err)) {
    ...
    if (status === 502 || status === 503 || status === 504) return `Call history backend is not responding (HTTP ${status}). Please check the backend service and try again.`
  ...
  if (err instanceof Error) {
    ...
    if (message.includes('timeout')) return 'Call history request timed out. The backend did not respond in time.'
    if (message.includes('network') || message.includes('failed to fetch')) return 'Call history backend is unreachable. Please check the backend service/network and try again.'
    return err.message
  }
  return 'Failed to load call history'
}

function groupCallsByCustomer(calls: CallRow[]): CustomerCallGroup[] {
  const map = new Map<string, CustomerCallGroup>()
  calls.forEach(call => {
...
function DetailField({ label, value, color, mono }: { label: string; value: string; color?: string; mono?: boolean }) {
  return (
    <div>
      <div style={{ fontSize: 9.5, fontWeight: 800, color: brand.faint, textTransform: 'uppercase', letterSpacing: 1.0, marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 12.5, fontWeight: 700, color: color || brand.ink, fontFamily: mono ? 'var(--font-mono)' : 'inherit' }}>{value}</div>
    </div>
  )
}
...
function CallsTableHeader() {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: tableColumns, minWidth: 1110, padding: '10px 12px', fontSize: 10.5, color: brand.faint, borderBottom: '1px solid var(--border)', textTransform: 'uppercase', letterSpacing: 0.9, fontWeight: 850 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', height: '100%', margin: '-10px -12px -10px 0', padding: '10px 12px 10px 16px' }}>
        <span style={{ borderRadius: 999, border: `1px solid ${brand.pink}88`, background: 'linear-gradient(135deg,rgba(251,11,140,0.18),rgba(251,11,140,0.04))', color: brand.pink, padding: '6px 12px', boxShadow: '0 0 26px rgba(251,11,140,0.18)', textShadow: '0 0 16px rgba(251,11,140,0.28)' }}>DISPOSITION</span>
      </div>
    </div>
  )
}
...
export default function Calls() {
  ...
  const dateRangeLabel = draftStartDate || draftEndDate ? `${draftStartDate || 'Any'} to ${draftEndDate || 'Any'}` : 'Date range'
  ...
  const filteredItems = useMemo(() => {
    ...
    const withoutInternalSipLegs = data.items.filter((item) => {
      ...
      const isDynamicCallerIdInternalSipLeg = item.direction === 'incoming' && campaignName.includes('__sip__') && (remoteName.startsWith('sip ') || remoteName.startsWith('sip:') || remoteNumber.startsWith('sip:')) && (remoteName.includes('@') || remoteNumber.includes('@'))
    ...
    return (
      <div key={call.id}>
        <div onClick={() => setExpandedId(isExpanded ? null : call.id)} style={{ display: 'grid', gridTemplateColumns: tableColumns, minWidth: 1110, padding: '11px 12px', fontSize: 12, borderTop: '1px solid var(--border)', alignItems: 'center', cursor: 'pointer', background: isExpanded ? 'rgba(251,11,140,0.05)' : 'transparent', transition: 'background 0.15s ease' }}>
          ...
          <div style={{ fontSize: 11, textAlign: 'right', color: brand.muted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{fmtDateTime(call.startedAt)}</div>
          <div style={{ display: 'grid', justifyItems: 'end', alignItems: 'center', gap: 5, minHeight: 42, margin: '-11px -12px -11px 0', padding: '11px 12px 11px 16px' }} onClick={e => e.stopPropagation()}>
            {currentDisposition && <span style={{ borderRadius: 999, border: '1px solid rgba(0,167,71,0.28)', background: 'rgba(0,167,71,0.08)', color: brand.green, padding: '3px 8px', fontSize: 9, fontWeight: 950, textTransform: 'uppercase', letterSpacing: 0.5 }}>{currentDisposition}</span>}
            <button type="button" onClick={() => setSelectedForDisposition(call)} style={{ minWidth: 112, height: 32, borderRadius: 999, border: `1px solid ${brand.pink}cc`, background: 'linear-gradient(135deg,rgba(251,11,140,0.42),rgba(128,87,215,0.22),rgba(0,245,160,0.12))', color: brand.ink, fontSize: 10.5, fontWeight: 950, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: 0.45, boxShadow: '0 0 24px rgba(251,11,140,0.28), inset 0 0 12px rgba(255,255,255,0.06)', textShadow: '0 1px 8px rgba(0,0,0,0.5)' }}>{call.disposition ? 'Edit disposition' : 'Set disposition'}</button>
          </div>
        </div>
        {isExpanded && (
          <div style={{ padding: '14px 20px 18px', borderTop: '1px solid rgba(251,11,140,0.18)', background: 'linear-gradient(180deg,rgba(251,11,140,0.04),transparent)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, minWidth: 1110 }}>
    ...
    <div className="ptdt-mobile-page ptdt-mobile-page-calls" style={{ padding: 24, color: brand.ink }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 14, alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8 }}>
          <button type="button" onClick={() => setDatePickerOpen(value => !value)} style={{ height: 36, display: 'flex', alignItems: 'center', gap: 8, borderRadius: 999, border: '1px solid var(--border-strong)', background: brand.surface, color: brand.ink, fontSize: 12, padding: '0 12px', cursor: 'pointer' }}><Calendar size={14} /><span>{dateRangeLabel}</span></button>
          {datePickerOpen && (
            <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', zIndex: 30, width: 278, borderRadius: 18, border: '1px solid var(--border-strong)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-lg)', padding: 12, display: 'grid', gap: 10 }}>
              <label style={{ display: 'grid', gap: 6, color: brand.muted, fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 0.7 }}>From<input type="date" value={draftStartDate} onChange={(e) => setDraftStartDate(e.target.value)} aria-label="Start date" style={{ height: 34, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)', color: brand.ink, fontSize: 12, outline: 'none', padding: '0 10px' }} /></label>
              <label style={{ display: 'grid', gap: 6, color: brand.muted, fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: 0.7 }}>To<input type="date" value={draftEndDate} onChange={(e) => setDraftEndDate(e.target.value)} aria-label="End date" style={{ height: 34, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)', color: brand.ink, fontSize: 12, outline: 'none', padding: '0 10px' }} /></label>
            </div>
          )}
        </div>
        <button type="button" onClick={applyDateRange} disabled={!draftStartDate && !draftEndDate} style={{ height: 36, borderRadius: 999, border: '1px solid rgba(0,122,77,0.92)', background: '#007a4d', color: '#ffffff', fontSize: 11, fontWeight: 950, textTransform: 'uppercase', letterSpacing: 0.9, padding: '0 17px', cursor: draftStartDate || draftEndDate ? 'pointer' : 'default', opacity: draftStartDate || draftEndDate ? 1 : 0.78, boxShadow: '0 0 18px rgba(0,122,77,0.22), inset 0 -10px 18px rgba(0,48,31,0.20)', textShadow: '0 1px 8px rgba(0,0,0,0.55)' }}>GO</button>
        {(draftStartDate || draftEndDate || startDate || endDate) && <button type="button" onClick={clearDateRange} style={{ height: 36, borderRadius: 999, border: `1px solid ${brand.pink}66`, background: 'rgba(251,11,140,0.10)', color: brand.pink, fontSize: 10, fontWeight: 950, textTransform: 'uppercase', letterSpacing: 0.7, padding: '0 14px', cursor: 'pointer' }}>CLEAR</button>}
      </div>
      ...
      <div style={{ marginTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, fontSize: 11, color: brand.muted, flexWrap: 'wrap' }}>
        <span>Showing {filteredItems.length} of {data?.total ?? 0} calls</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <button type="button" onClick={() => changePage(page - 1)} disabled={!hasPrev} style={{ width: 30, height: 30, borderRadius: 999, border: '1px solid var(--border-strong)', background: hasPrev ? brand.surface : brand.panel, color: brand.ink, display: 'grid', placeItems: 'center', cursor: hasPrev ? 'pointer' : 'default', opacity: hasPrev ? 1 : 0.55 }}><ChevronLeft size={16} /></button>
          <button type="button" onClick={() => changePage(page + 1)} disabled={!hasNext} style={{ width: 30, height: 30, borderRadius: 999, border: '1px solid var(--border-strong)', background: hasNext ? brand.surface : brand.panel, color: brand.ink, display: 'grid', placeItems: 'center', cursor: hasNext ? 'pointer' : 'default', opacity: hasNext ? 1 : 0.55 }}><ChevronRight size={16} /></button>
        </div>
      </div>

      <CallDispositionModal open={Boolean(selectedForDisposition) && Number.isFinite(selectedDispositionCallId)} callId={Number.isFinite(selectedDispositionCallId) ? selectedDispositionCallId : null} contactName={selectedForDisposition?.remoteName} contactNumber={selectedForDisposition?.remoteNumber} defaultDisposition={selectedForDisposition?.disposition} defaultNotes={selectedForDisposition?.notes} defaultCallbackAt={selectedForDisposition?.callbackAt} onClose={() => setSelectedForDisposition(null)} onSaved={() => { setSelectedForDisposition(null); void queryClient.invalidateQueries({ queryKey: ['calls'], exact: false }); void queryClient.refetchQueries({ queryKey: ['calls'], exact: false, type: 'active' }) }} />
    </div>
  )
}

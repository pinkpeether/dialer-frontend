...
const STATUS_COLORS: Record<string, { color: string; bg: string }> = { PENDING: { color: COL_GOLD, bg: 'rgba(240,185,11,0.12)' }, CALLING: { color: COL_PINK, bg: 'rgba(251,11,140,0.10)' }, ANSWERED: { color: COL_GREEN, bg: 'rgba(0,167,71,0.10)' }, DONE: { color: COL_GREEN, bg: 'rgba(0,167,71,0.10)' }, NO_ANSWER: { color: COL_GOLD, bg: 'rgba(240,185,11,0.12)' }, BUSY: { color: COL_DANGER, bg: 'rgba(239,68,68,0.12)' }, DNC: { color: 'var(--text-3)', bg: 'var(--bg-2)' } }
...
export default function Contacts() {
  ...
  const confirmDeleteContact = (contact: Record<string, unknown>) => { const contactId = contact.id as number; const label = String(contact.name || contact.phone || 'this contact'); setDialog({ tone: 'confirm', title: 'Delete contact?', message: `This will remove ${label} from the contact list.`, confirmLabel: 'Delete Contact', onConfirm: async () => { setDialog(null); try { await deleteContact(contactId); setDialog({ tone: 'success', title: 'Contact deleted', message: 'The contact has been deleted.' }) } catch (err) { setDialog({ tone: 'error', title: 'Cannot delete contact', message: err instanceof Error ? err.message : 'Failed to delete contact' }) } } }) }

  const renderContactRow = (c: ContactRecord, i: number) => { const sc = STATUS_COLORS[c.status as string] || STATUS_COLORS.PENDING; const campaign = campaigns.find(item => Number(item.id) === Number(c.campaignId)); return <motion.tr key={c.id as number} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.015, 0.18) }} className="table-row" style={{ borderBottom: '1px solid var(--border)' }}><td className="mono" style={{ padding: '14px 16px', fontSize: 12, color: 'var(--text-3)' }}>{String(i + 1).padStart(2, '0')}</td><td style={{ padding: '14px 16px' }}><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--grad-brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: '#fff', boxShadow: '0 0 12px rgba(251,11,140,0.32)' }}>{String(c.name || '?').charAt(0).toUpperCase()}</div><span style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)' }}>{c.name as string}</span></div></td><td className="mono" style={{ padding: '14px 16px', fontSize: 13, color: 'var(--text-2)' }}>{c.phone as string}</td><td className="mono" style={{ padding: '14px 16px', fontSize: 12, color: 'var(--text-3)' }}>{String(campaign?.name || `#${c.campaignId as number}`)}</td><td style={{ padding: '14px 16px' }}><span className="badge" style={{ color: sc.color, background: sc.bg, border: `1px solid ${sc.color}` }}>{c.status as string}</span></td><td style={{ padding: '14px 16px' }}><button onClick={() => confirmDeleteContact(c)} style={{ background: 'transparent', border: '1px solid rgba(239,68,68,0.32)', borderRadius: 'var(--radius-sm)', padding: '6px 10px', cursor: 'pointer', color: COL_DANGER, display: 'inline-flex', alignItems: 'center' }}><Trash2 size={13} /></button></td></motion.tr> }

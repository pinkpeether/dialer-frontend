...
const getStatusColor = (status: string) => status === 'ANSWERED' ? '#16a34a' : status === 'RINGING' ? '#f59e0b' : status === 'INITIATED' ? '#2563eb' : '#64748b'
...
function ActiveCallsTable({ calls }: { calls: LiveCallWallItem[] }) {
  return <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 880 }}><thead><tr>{['Call', 'Status', 'Contact', 'Campaign', 'Agent', 'Region', 'Started'].map(header => <th key={header} style={{ textAlign: 'left', padding: '10px 8px', fontSize: 12, color: 'var(--text-3)', borderBottom: '1px solid var(--border)' }}>{header}</th>)}</tr></thead><tbody>{calls.map(call => <tr key={call.id}><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)', fontWeight: 800 }}>#{call.id}</td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)' }}><span style={{ background: getStatusColor(call.status), color: '#fff', borderRadius: 999, padding: '4px 10px', fontSize: 12, fontWeight: 800 }}>{call.status}</span></td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)' }}><div style={{ color: 'var(--text)', fontWeight: 800 }}>{call.contact?.name || 'Unknown'}</div><div style={{ color: 'var(--text-3)', fontSize: 12 }}>{call.remoteNumber || call.contact?.phone || 'No number'}</div></td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)' }}>{call.campaign?.name || '—'}</td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)' }}>{call.agent?.name || 'Unassigned'}</td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)' }}>{call.region?.label || 'Unknown'}</td><td style={{ padding: '12px 8px', borderBottom: '1px solid var(--border)', color: 'var(--text-3)', fontSize: 12 }}>{new Date(call.startedAt).toLocaleString()}</td></tr>)}</tbody></table></div>
}
...
export default function LiveMonitoringAdvanced() {
  ...
  const fetchOverview = useCallback(async (options: { silent?: boolean } = {}) => {
    ...
    try {
      ...
      setData(await liveMonitoringAdvancedAPI.getOverview(parsedCampaignId, { silent }))
      hasVisibleDataRef.current = true
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed to load live calls') }
  ...
  return <div className="ptdt-page ptdt-pro-page">
    <div className="ptdt-page-header ptdt-pro-hero">
      <div><div className="eyebrow pink" style={{ marginBottom: 12 }}><Activity size={12} /> Live Call Operations</div><h1 className="ptdt-page-title">Live <span className="gradient-brand-text">Calls</span></h1><p className="ptdt-page-desc">Live call wall, regional call buckets, hourly activity, simultaneous-call counter, answer-rate tracking, and dialing guidance.</p></div>

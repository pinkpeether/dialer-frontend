...
import { ChevronDown, ChevronRight, Copy, Eye, Megaphone, Pause, Play, Plus, Trash2, X } from 'lucide-react'
...
const campaignFieldLabelStyle: CSSProperties = {
  color: 'var(--text-2)',
  fontSize: 12,
  fontWeight: 900,
}
...
const accountForCampaign = (campaign: CampaignRecord): CustomerAccount => {
  ...
  return {
    ...
    name: getText(account?.name, 'Unassigned Customer'),
    code: getText(account?.code, '—'),
    status: getText(account?.status, '—'),
  }
}
    ...
    <div className="ptdt-mobile-page ptdt-mobile-page-campaigns" style={{ padding: '32px 36px', maxWidth: 1600, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, gap: 18, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ fontSize: 14.5, color: 'var(--text-3)' }}>Create, inspect, and orchestrate customer-grouped outbound campaigns.</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="glass" style={{ padding: 24, marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 14, alignItems: 'start' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          {groupedCampaigns.map((group, groupIndex) => {
            ...
            const active = group.campaigns.filter(campaign => campaign.status === 'ACTIVE').length
            const draft = group.campaigns.filter(campaign => campaign.status === 'DRAFT').length
            const paused = group.campaigns.filter(campaign => campaign.status === 'PAUSED').length
            return (
              <div key={group.key} className="glass" style={{ overflow: 'hidden' }}>
                <button type="button" onClick={() => toggleGroup(group.key)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: 16, background: 'transparent', border: 0, borderBottom: isOpen ? '1px solid var(--border)' : 0, color: 'var(--text)', cursor: 'pointer', textAlign: 'left' }}>
                  ...
                  <span className="badge" style={{ color: 'var(--pink)', background: 'rgba(251,11,140,.10)', border: '1px solid rgba(251,11,140,.28)' }}>{group.campaigns.length} Campaigns</span>
                  <span className="badge" style={{ color: COL_GREEN, background: 'rgba(0,167,71,.10)', border: '1px solid rgba(0,167,71,.28)' }}>{active} Active</span>
                  <span className="badge" style={{ color: COL_GOLD, background: 'rgba(240,185,11,.12)', border: '1px solid rgba(240,185,11,.28)' }}>{paused} Paused</span>
                  <span className="badge" style={{ color: 'var(--text-3)', background: 'var(--bg-2)', border: '1px solid var(--border)' }}>{draft} Draft</span>
                </button>
                {isOpen && (
...
function ActionBtn({ icon, label, onClick, color, bg, disabled }: { icon: ReactNode; label: string; onClick?: () => void; color: string; bg: string; disabled?: boolean }) {
  return (
...
function IconBtn({ icon, onClick, color, border, disabled }: { icon: ReactNode; onClick?: () => void; color: string; border?: string; disabled?: boolean }) {
  return (
